#!/usr/bin/env python3
"""
Phase 7.8-v3 — Build the frozen plant-level Phase-7 analytical dataset
...
def numeric_or_simple_formula(ws, row: int, col: int) -> tuple[float, str]:
    """
    Return a numeric cell value, or evaluate the narrow source formula pattern
    used by the portable TDS columns (for example '=C5/2').

    This does not invent a transformation: it reproduces the formula explicitly
    stored in the workbook. Only one-cell arithmetic with *, /, +, or - and a
    numeric constant is accepted. Anything else remains missing.
    """
    v = ws.cell(row, col).value
    direct = to_float(v)
    if not pd.isna(direct):
        return direct, "RAW_NUMERIC"

    s = clean(v).replace(" ", "")
    m = re.fullmatch(
        r"=\$?([A-Za-z]{1,3})\$?(\d+)([*/+-])([-+]?(?:\d+(?:\.\d*)?|\.\d+))",
        s,
    )
    if not m:
        return math.nan, "MISSING_OR_UNSUPPORTED"

    ref_col_letters, ref_row_s, op, const_s = m.groups()
    from openpyxl.utils.cell import column_index_from_string

    ref_row = int(ref_row_s)
    ref_col = column_index_from_string(ref_col_letters)
    ref = to_float(ws.cell(ref_row, ref_col).value)
    if pd.isna(ref):
        return math.nan, "FORMULA_REFERENCE_MISSING"

    const = float(const_s)
    try:
        if op == "/":
            if const == 0:
                return math.nan, "FORMULA_DIVIDE_BY_ZERO"
            result = ref / const
        elif op == "*":
            result = ref * const
        elif op == "+":
        ...
        return math.nan, "FORMULA_EVALUATION_FAILED"

    return float(result), "SOURCE_FORMULA_EVALUATED"
...
def extract_portable(ws, exp: int) -> tuple[list[dict], list[str]]:
    ...
    for b in blocks:
        for rr, dr in date_by_row.items():
            tds_value, tds_provenance = numeric_or_simple_formula(
                ws, rr, b["start_col"] + 2
            )
            vals = {
            ...
            rows.append({
                ...
                "tds_value_provenance": tds_provenance,
                **vals,
            })
...
def find_total_weight_header_near(ws, domain_row: int) -> tuple[int, int] | None:
    """
    Locate the total-weight column robustly.

    The three harvest workbooks do not use one identical spelling/layout.
    Accept any nearby header whose normalized text contains both "total" and
    "weight" (for example "Total weight", "Total Weight (g)", etc.) instead
    of requiring one exact token.
    """
    candidates = []
    lo = max(1, domain_row - 8)
    hi = min(ws.max_row, domain_row + 8)

    for r in range(lo, hi + 1):
        for c in range(1, ws.max_column + 1):
            raw = clean(ws.cell(r, c).value)
            tok = norm_token(raw)
            if "total" in tok and "weight" in tok:
                # Prefer a header closest to the domain row. On equal distance,
                # prefer the earlier row for deterministic behavior.
                candidates.append((abs(r - domain_row), r, c, raw))

    if not candidates:
        return None

    candidates.sort(key=lambda x: (x[0], x[1], x[2]))
    _, r, c, _ = candidates[0]
    return r, c
...
def find_plant_number_col(ws, header_row: int) -> int | None:
    for c in range(1, ws.max_column + 1):
        ...
        if (
            tok in {"plantno", "plantnumber", "plant", "no"}
            or tok.startswith("plantno")
            or tok.startswith("plantnumber")
        ):
            return c
    return None
...
def main() -> int:
    ...
    report = [
        "PHASE 7.8-v3 — PLANT-LEVEL ANALYTICAL DATASET CONSTRUCTION",
        "=" * 118,
        f"Archive: {ARCHIVE}",
        ...
        "P7.8-v2 harvest fix: total-weight headers are matched robustly by normalized",
        "presence of both 'total' and 'weight', rather than a small exact-name list.",
        "P7.8-v3 TDS fix: simple TDS formulas stored in the workbook (e.g. EC/2) are",
        "evaluated from their referenced raw cells with provenance; raw workbooks remain unchanged.",
        "",
        "SOURCE EXTRACTION",
        "-" * 118,
    ]
    ...
    manifest = {
        ...
        "p78v2_harvest_header_fix": "normalized header contains both total and weight",
        "p78v3_tds_formula_fix": "evaluate only simple arithmetic formulas explicitly stored in source workbook",
